...
const updateProfile = authenticatedAction.create(
  updateProfileSchema,
  async (
    { firstName, lastName, email, phone, password, profile_picture, bio },
    context
  ) => {
    ...
    try {
      ...
      if (firstName?.trim() || lastName?.trim() || avatar_url || bio?.trim()) {
        // ← Added `bio` here
        ...
        await db.transaction(async (tx) => {
          if (firstName?.trim() || lastName?.trim() || bio?.trim()) {
            // ← Added `bio` check
            const fullName = `${
              firstName?.trim() || context.user_metadata?.firstName || ""
            } ${
              lastName?.trim() || context.user_metadata?.lastName || ""
            }`.trim();
            await tx
              .update(users_additional_info)
              .set({
                full_name: fullName,
                bio: bio?.trim() || context.user_metadata?.bio || "",
              }) // ← Added `bio`
